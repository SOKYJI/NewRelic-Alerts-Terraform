...
# SystemCheckFailed 발생
resource "newrelic_nrql_alert_condition" "ec2_system_check_failed" {
  policy_id = var.policy_id
  name      = "SystemCheckFailed 발생"
  ...
  critical {
    operator              = "above"
    threshold             = var.system_check_failed_threshold
    threshold_duration    = 300
    threshold_occurrences = "ALL"
  }
}

# InstanceCheckFailed 발생
resource "newrelic_nrql_alert_condition" "ec2_instance_check_failed" {
  policy_id = var.policy_id
  name      = "InstanceCheckFailed 발생"
  ...
  critical {
    operator              = "above"
    threshold             = var.instance_check_failed_threshold
    threshold_duration    = 300
    threshold_occurrences = "ALL"
  }
}
...
# EBSCheckFailed 발생
resource "newrelic_nrql_alert_condition" "ec2_ebs_check_failed" {
  policy_id = var.policy_id
  name      = "EBSCheckFailed 발생"
  ...
  critical {
    operator              = "above"
    threshold             = var.ebs_check_failed_threshold
    threshold_duration    = 300
    threshold_occurrences = "ALL"
  }
}
